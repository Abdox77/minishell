...
/*   Updated: 2024/07/01 22:35:12 by amohdi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	handle_quote_in_expanded_line(t_exec *exec, char *line,
		char **expanded_line, t_iter_vars *vars)
{
	char	*buff;

	buff = NULL;
	*expanded_line = ft_strjoin(*expanded_line, ft_substr(line, vars->i,
				vars->len + 1));
	vars->i += vars->len + 1;
...
static void	handle_consecutive_dollarsign(char *line, char **expanded_line,
		t_iter_vars *vars)
{
	if (line[vars->i + 1] == '$')
	{
...
{
	if (*s == '$' && (*(s + 1) == '$' || (ft_isalpha(*(s + 1)) == 0 && *(s
					+ 1) != '_')))
		return (TRUE);
	return (FALSE);
}

static void	handle_single_dollarsign(t_exec *exec, char *line,
		char **expanded_line, t_iter_vars *vars)
{
	char	*buff;
	char	*env_variable;
	...
	while (is_valid_char_for_expansion(line[vars->i + vars->len]) == TRUE
		&& is_space(line[vars->i + vars->len]) == FALSE)
		++(vars->len);
	if (vars->len)
	{
...
char	*expand_in_heredoc(t_exec *exec, char *line)
{
	...
	if (!line)
		return (NULL);
	init_iter_vars(&vars, &expanded_line);
	while (line[vars.i])
	{
		vars.len = 0;
		while (is_valid_char_for_expansion(line[vars.i + vars.len]) == TRUE)
